...
- **按技能分配模型**：check 使用 Haiku（轻量 CI 检测，节省 token），commit/push/pr/version/hud 使用 Sonnet（语义分析与内容生成）；各 SKILL.md 通过 `model` frontmatter 字段声明，随技能激活自动切换
...
# 测试完整管道（push 已含 version bump，check 单独按需使用）
