...
```
根目录
├── .claude-plugin/marketplace.json  # 项目级 marketplace 注册文件（多插件市场：smart + plan-fidelity-guard）
...
assets/i18n/cn/plan-fidelity-guard/   # CN 镜像（仅供阅读，结构与 plugins/plan-fidelity-guard/ 对称）
└── hooks/
    ├── hooks.json
    └── plan-guard.py                  # 计划保真守卫（UserPromptSubmit）
...
plugins/plan-fidelity-guard/      # 独立插件：防止已批准设计被实现计划静默裁剪
├── .claude-plugin/plugin.json    # 插件元数据
└── hooks/
    ├── hooks.json                # UserPromptSubmit（注入清单）+ Stop（语义比对）
    └── plan-guard.py             # 计划前注入保真清单的命令型 hook
```

本仓库是一个**多插件市场**：`plugins/` 下每个子目录是一个独立插件，均在 `.claude-plugin/marketplace.json` 注册。

语言版本组织：EN 文件在 `plugins/<plugin>/`（被加载），CN 文件在 `assets/i18n/cn/<plugin>/`（仅供阅读参考），文件名完全相同。
...
- **按技能分配模型**：check 使用 Haiku（轻量 CI 检测，省钱提速），commit / push / pr 使用 Sonnet（语义分析与内容生成）；通过各 SKILL.md 的 `model` frontmatter 字段声明。注意：嵌套引用（pr → push → commit）执行时以父 skill 的模型为准，故 check 的 Haiku 仅在其作为独立 skill 被激活时生效
