...
- **기본 형식 (위 1, 2가 모두 해당되지 않을 때 사용):**
  ...
  - 언어: `git log`의 최근 commit message에서 언어를 추론하고 일관성을 유지합니다. 저장소에 히스토리가 없으면 기본값은 영어입니다. 프로젝트 `CLAUDE.md` 또는 `CLAUDE.local.md`의 명시적 언어 설정이 최우선입니다.
