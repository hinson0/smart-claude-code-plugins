...
- `git diff`와 `git diff --staged` 내용을 읽고, **파일별 구조화 분석**을 수행합니다:
  ...
  b. **독립적 목적별 그룹화 후 type으로 검증**:
     - 동일한 목적의 파일 → 한 그룹.
     - 목적이 다른 파일 → 다른 그룹 (type이 같더라도).
     - 그룹 간 다른 type → 분할이 필수임을 확인.
     - 하나의 그룹 내에서 다른 type → 해당 그룹을 추가 분할해야 함.
     - 총 1개 그룹 → **단일 커밋**.
     - 2개 이상 그룹 → **다중 커밋** (필수, 예외 없음).
  c. **다중 커밋인 경우: 그룹화 계획을 터미널에 출력**:
     Group 1 (refactor): src/sheet.tsx, src/layout.tsx
     Group 2 (fix): src/api/entry.ts
     Group 3 (chore): app.json, .prettierrc
  d. 이 그룹화를 가지고 4단계로 진행합니다.
- **분할 규칙 (엄격히 적용)**:
...
- **기본 형식 (프로젝트 CLAUDE.md에 사용자 정의 commit 형식이 정의되지 않은 경우 사용):**
  ...
  - 언어: 사용자의 소통 언어를 따릅니다 (예: 사용자가 한국어로 작성하면 한국어, 영어면 영어). 불확실한 경우 기본값은 영어.
