...
- **语言优先级（从高到低）**：
  1. 项目 `CLAUDE.md` / `CLAUDE.local.md` 中**明确针对 git commit message 语言**的设定（如"commit message 用中文"）
  2. 从 `git log` 近期 commit message 推断的语言（保持一致）
  3. 默认英文（仓库无历史记录时）
  > ⚠️ 通用的对话语言偏好（如"用中文回复"）**不影响** commit message 语言。
