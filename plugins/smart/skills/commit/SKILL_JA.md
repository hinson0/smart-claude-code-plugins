...
- `git diff` と `git diff --staged` の内容を読み取り、**ファイル単位の構造化分析**を実行します：
  ...
  b. **フェーズ1 — type による機械的分割**（判断不要）：
     - `Type` 列に基づいてファイルを厳密にグループ化します。異なる type のファイルが同じグループに入ることは**不可能**です。これは交渉の余地がなく、意味的判断は不要です。
     - 例：テーブルに `refactor`、`fix`、`chore` の3種類の type がある場合 → 最低3グループ、type ごとに1グループ。
  c. **フェーズ2 — 各 type グループ内で目的別に細分化**（セマンティック分析）：
     - フェーズ1の各 type グループ内で、ファイルが異なる独立した目的を持つかどうかを確認します。
     - 同じ目的 → 1つのグループのまま維持。
     - 異なる目的 → 別々のグループに分割（例：無関係な2つの `fix` 変更は2グループになる）。
  d. **最終グループ数を集計**：
     - グループが合計1つ → **単一コミット**。
     - グループが2つ以上 → **複数コミット**（必須、例外なし）。
  e. **複数コミットの場合：グループ分けプランをターミナルに出力**：
     ```
     Group 1 (refactor): src/sheet.tsx, src/layout.tsx
     Group 2 (fix): src/api/entry.ts
     Group 3 (chore): app.json, .prettierrc
     ```
  f. **このグループ分けを携えて第4ステップに進みます**。
...
- **デフォルト形式（プロジェクトの CLAUDE.md にカスタム commit 形式が定義されていない場合に使用）：**
  ...
  - 言語：デフォルトは英語。プロジェクトの `CLAUDE.md` または `CLAUDE.local.md` で git commit message の言語が明示的に指定されている場合のみ、指定された言語を使用します。
  - 「なぜ変更したか」に焦点を当て、曖昧な記述を避けること
- **プロジェクトによるオーバーライド：** プロジェクトの `CLAUDE.md` または `CLAUDE.local.md` にカスタム git commit message 形式または言語要件が定義されている場合、プロジェクトのルールに従い、上記デフォルトは無視すること。
- 単一 feature：
