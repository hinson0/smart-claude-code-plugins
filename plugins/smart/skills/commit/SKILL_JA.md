...
- **デフォルト形式（上記 1、2 のいずれも該当しない場合に使用）：**
  ...
  - 言語：`git log` の最近の commit message から言語を推測し、一貫性を保つこと。リポジトリに履歴がない場合はデフォルトで英語。プロジェクトの `CLAUDE.md` または `CLAUDE.local.md` での明示的な言語設定が最優先。
