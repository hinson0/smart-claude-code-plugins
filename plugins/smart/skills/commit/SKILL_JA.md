...
- **デフォルト形式（プロジェクトの CLAUDE.md にカスタム commit 形式が定義されていない場合に使用）：**
  ...
  - 言語：デフォルトは英語。プロジェクトの `CLAUDE.md` または `CLAUDE.local.md` で commit message の言語が明示的に指定されている場合のみ、指定された言語を使用します。
  - 「なぜ変更したか」に焦点を当て、曖昧な記述を避けること
- **プロジェクトによるオーバーライド：** プロジェクトの `CLAUDE.md` または `CLAUDE.local.md` にカスタム commit message 形式または言語要件が定義されている場合、プロジェクトのルールに従い、上記デフォルトは無視すること。
- 単一 feature：
