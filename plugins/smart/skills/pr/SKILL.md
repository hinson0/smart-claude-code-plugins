...
1. Use the supplied branch argument as the target without confirmation. If omitted, ask “Create a PR targeting main by default?” in the user's language and wait for explicit confirmation before proceeding. Silence is not confirmation.
...
4. Run project-required checks and review the full diff against the target. Stop on failure or blocking findings. If there is no PR diff, report that and stop.
