...
- PR タイトル、Summary、Test Plan の言語を決定：フェーズ 1 で commit skill が生成した commit message と同じ言語を使用します（commit skill の言語ルールが唯一の言語決定源です）。フェーズ 1 がスキップされた場合（変更なし）、同じルールを適用：デフォルトは英語とし、CLAUDE.md / CLAUDE.local.md で git commit message の言語が明示的に指定されている場合のみその言語を使用します。Section headers（## Summary、## Commits、## Test Plan）は常に英語のまま、commit messages は翻訳しない。
