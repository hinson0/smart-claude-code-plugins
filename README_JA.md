...
| `/smart:config` | 自動アクションの設定（タスク完了後の自動 commit/push） |

---

## 自動アクション（Auto-Action）

Claude にタスク完了後の自動コミット・プッシュをさせたい場合、一度設定するだけです：
...
選択肢：
- **off** — 無効（デフォルト、手動のみ）
- **commit** — タスク完了後に自動 commit（ローカルのみ）
- **push** — タスク完了後に自動 commit + push

設定はプロジェクトごとに `.claude/smart.local.md` に保存され、次のセッションから有効になります。
