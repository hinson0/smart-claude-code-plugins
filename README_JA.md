...
Codex 形式のプラグインです。コードを書き終えたら、ひとこと言うだけ — 自動でチェック、コミット、プッシュし、`main` ブランチへの Pull Request を作成します。追加の操作は一切不要です。`push` の一言で、複数 feature の自動分割、commit message の生成、プッシュまで完了：
...
まずリポジトリを clone し、ローカル Codex マーケットプレイスとして登録します：
...
次に `smart` マーケットプレイスからプラグインをインストールします：
