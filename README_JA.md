...
- "PRを作って" / "create PR" → check + commit + version + push + PR
...
| `/smart:version [ベースブランチ]` | コミットを分析しバージョンをバンプ（plugin.json / package.json / pyproject.toml を自動検出） |
...
/smart:pr
    ...
    │
    ├── 3. version — バージョンファイルを検出（plugin.json / package.json / pyproject.toml）
    │                コミットタイプを分析 → セマンティックバージョンをバンプ（major/minor/patch）
    │                （モノレポ：ファイルを所属パッケージにマッピングし独立バンプ）
    │
    ├── 4. push    — origin にプッシュ
    │                （remote 未設定の場合は自動で GitHub リポジトリを作成し紐付け）
    │
    └── 5. pr      — タイトルと本文を自動生成し、Pull Request を作成
