...
- **Learning Mode** — `/smart:learning 1` turns on a co-coding mode where *you* hand-write the meaningful parts. The split is a layer × kind grid — frontend/backend/db × boilerplate/business — where each number is *your* hand-written share (0 = Claude writes it all, 100 = you write it all; defaults: frontend 0/30, backend 30/70, db 0/100). Per task, Claude lands its own share to disk and prints your whole share to the console as a "stuck? one way to do it" reference (framing first, answer last) so you never need a second round-trip, then reviews what you land before moving on. The toggle and the grid live in `.smart/settings.json` (`learning` + `learning_ratios`, shared with distill, under the git-ignored `.smart/` dir); enabling injects the rules into `.claude/CLAUDE.local.md` so they persist across every session, `/smart:learning config backend.business=100 frontend.boilerplate=0` retunes a cell, and `/smart:learning 0` removes the block.
...
| `/smart:learning [0\|1\|config]` | Toggle learning mode — *you* hand-write part of the code; per-cell shares on a layer × kind grid (frontend/backend/db × boilerplate/business) configurable in `.smart/settings.json`. Claude prints your share to the console as a fallback, then reviews what you land. `1`=on, `0`=off, `config layer.kind=NN`=retune (e.g. `backend.business=100`), empty=status. Rules persisted in `.claude/CLAUDE.local.md` |
