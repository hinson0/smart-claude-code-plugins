...
- **HUD / Statusline 설치기** — 한 줄 명령어로 모델, Git 브랜치, 컨텍스트 사용량, 속도 제한, 시스템 리소스, 도구 호출 통계를 표시하는 상태 표시줄을 설치합니다. 설치 / 제거 / 복원을 지원합니다.
...
| 명령어 | 동작 |
|--------|------|
| `/smart:hud` | 설치 (기존 상태 표시줄 자동 백업) |
| `/smart:hud rm` | 상태 표시줄 제거 및 기본값 복원 |
| `/smart:hud rewind` | 백업에서 이전 상태 표시줄 복원 |
...
어떤 플러그인이 가장 많은 컨텍스트 윈도우를 차지하는지 진단합니다.

```
"analyze context" / "컨텍스트 분석" / "어떤 플러그인이 가장 큰지"
```

- `~/.claude/settings.json`에서 활성화된 플러그인 목록 읽기
- 각 플러그인 캐시 디렉토리의 모든 `.md` 파일 크기 측정
- 크기와 비율이 포함된 Markdown 순위 테이블 출력
- 3KB 미만의 플러그인은 "Others"로 통합
- 하단에 총 컨텍스트 윈도우 점유 비율 추정
