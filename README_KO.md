...
- **자동 버전 범프** — 버전 파일(`plugin.json`, `package.json` 또는 `pyproject.toml`)을 자동 감지하고, 커밋 유형을 분석하여 푸시 전에 시맨틱 버전을 자동 범프합니다. 모노레포에서는 변경된 파일을 해당 패키지에 매핑하여 각각 독립적으로 범프합니다.
...
- "PR 만들어" / "create PR" → check + commit + version + push + PR
...
| `/smart:version [베이스 브랜치]` | 커밋을 분석하고 버전 범프 (plugin.json / package.json / pyproject.toml 자동 감지) |
...
자동으로 수행됩니다: CI 설정 감지 후 로컬 검사 실행 → 스마트 커밋 → 버전 범프 → 푸시 → GitHub에서 PR 생성.
...
/smart:pr
    ...
    │
    ├── 3. version — 버전 파일 감지 (plugin.json / package.json / pyproject.toml)
    │                커밋 유형 분석 → 시맨틱 버전 범프 (major/minor/patch)
    │                (모노레포: 파일을 소속 패키지에 매핑하여 독립 범프)
    │
    ├── 4. push    — origin으로 푸시
    │                (remote 미설정 시 자동으로 GitHub 저장소 생성 및 연결)
    │
    └── 5. pr      — 제목과 본문을 자동 생성하여 Pull Request 생성
