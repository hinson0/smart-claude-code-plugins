...
Stop Hook을 통해 구현: CC가 작업을 완료하면 자동으로 commit 또는 push를 트리거합니다.

**설정 방법**: `/smart:config`를 실행하여 대화형으로 전환 (기본값: 꺼짐).

**흐름**:

```
CC 작업 완료 → Stop 이벤트 발생
                    ↓
            auto-commit.sh 실행
                    ↓
        .claude/smart.local.md 읽기
                    ↓
            auto_action 값?
           /        |        \
         off      commit     push
