...
Smart는 `ask`, `clean-branches`, `close-issue`, `code-simplifier`, `commit`, `generate-wiki`, `github-skills-pdf`,
`help`, `html`, `hud`, `learning`, `local`, `matt-implement-all-tickets`,
`my-weekly`, `one-by-one`, `pair-write`, `pr`, `show`의 열여덟 개 skill을 포함합니다.
...
| `/smart:clean-branches [branch]` | 대상에 완전히 병합된 로컬·원격 브랜치 삭제. 인수 없으면 기본 `main` 확인 후 대기. 보호 및 worktree 사용 중인 브랜치는 유지 |
...
`commit`, `pr`, `clean-branches`는 Codex 메타데이터에서도 암시적 호출을 비활성화하며 사용자가 명시적으로 호출해야 합니다.
