...
- **學習模式** — `/smart:learning 1` 開啟協作編碼模式：由*你*親手編寫程式碼中有意義的部分。劃分是一張 層×類 網格——frontend/backend/db × 樣板/業務——每個數字是*你*親手編寫的占比（0 = Claude 全寫，100 = 你全寫；預設：frontend 0/30、backend 30/70、db 0/100）。每個任務裡 Claude 把自己那份直接落盤，並把你那份整份作為「卡住再看」的參考解法印到主控台（框架在前、答案在後），讓你無需再起一輪對話，然後審查你落盤的程式碼再繼續。開關與網格保存在 `.smart/settings.json`（`learning` + `learning_ratios`，與 distill 共用，位於已 git-ignore 的 `.smart/` 目錄）；開啟時把規則注入 `.claude/CLAUDE.local.md` 使其每個工作階段持續生效，`/smart:learning config backend.business=100 frontend.boilerplate=0` 調單格，`/smart:learning 0` 移除該塊。
...
| `/smart:learning [0\|1\|config]` | 切換學習模式——由*你*親手寫一部分程式碼；層×類 網格（frontend/backend/db × 樣板/業務）各格占比可在 `.smart/settings.json` 設定。Claude 把你那份印到主控台作後備，再審查你落盤的程式碼。`1`=開，`0`=關，`config layer.kind=NN`=調單格（如 `backend.business=100`），留空=狀態。規則持久化在 `.claude/CLAUDE.local.md` |
