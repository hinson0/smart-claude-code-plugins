...
- 在提交或推送任何内容之前先运行本地检查。
- 若任一检查失败，立即停止——不再进行提交、版本升级、推送或创建 PR。报告失败的检查项及其修复命令。
- 若 check skill 报告无变更或未检测到 CI 配置，跳过本阶段并继续。
...
- 若工作区干净（无任何变更）且无未推送的 commit，跳过阶段二，直接进入阶段三。
...
- 确定 PR 标题、Summary 和 Test Plan 的语言：与阶段二中 commit skill 生成的 commit message 使用相同语言（commit skill 的语言规则为唯一语言决策源）。若阶段二被跳过（无变更），沿用相同规则：默认英文，除非 AGENTS.md / CLAUDE.md / CLAUDE.local.md 明确规定了 git commit message 语言。Section headers（## Summary、## Commits、## Test Plan）始终保持英文，commit messages 不翻译。
...
- **语言**：使用上面步骤 1 中确定的语言。Section headers（## Summary, ## Commits, ## Test Plan）始终保持英文。
- **标题**：
...
1. 阶段二使用的所有 commit message（若有改动）。
