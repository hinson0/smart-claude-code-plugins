...
- 确定 PR 标题、Summary 和 Test Plan 的语言：与阶段二中 commit skill 生成的 commit message 使用相同语言（commit skill 的语言规则为唯一语言决策源）。若阶段二被跳过（无变更），沿用相同规则：默认英文，除非 CLAUDE.md / CLAUDE.local.md 明确规定了 git commit message 语言。Section headers（## Summary、## Commits、## Test Plan）始终保持英文，commit messages 不翻译。
