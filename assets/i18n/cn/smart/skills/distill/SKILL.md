---
description: 当用户说"蒸馏这次对话"、"distill"、"把刚才聊的整理到知识库"、"按主题落盘这次会话"、"总结当前 CC 输出"、"把我提过的问题和答案归档"、"提炼会话主题"、"把这次聊天落盘"时使用。该 skill **不读源目录文件**,直接从**当前 CC 会话上下文**(用户提问 + Claude 输出)抽取有价值的问答对,按主题语义聚类生成 kebab-case 主题键,仅与**解析出的目标目录**做三态比对(重复/新增/差分),落盘到该目录。目标目录在运行时解析:优先读 .smart/settings.json(项目)或 ~/.smart/settings.json(全局),没有则用 AskUserQuestion 问一次并保存,之后静默;默认 .smart/knowledges/,支持 {date} 占位符,向后兼容个人当日知识库。**绝不触碰目标目录之外的任何路径**。已 review 文件(`.printed.md` 或有同名 pdf)豁免比对。
...
落盘目录由 Step 0 解析确定,是本次运行的唯一真相源。所有读写都限定在 `<目标目录>` 内,本 skill 绝不向上回溯到父目录或横向进入兄弟目录。当 `<目标目录>` 为个人当日知识库(`~/knowledges/md/{date}/`)时,其日期在解析时即固定,之后不可被覆盖——与全局 CLAUDE.md 约定一致。
